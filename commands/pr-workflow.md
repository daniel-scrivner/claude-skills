...
3. If any check produces new feedback, repeat Step 5
...
```
Call mcp__github__pull_request_read with method "get" and verify:
  - mergeable_state == "clean"  ← THIS IS THE GATE
  - merged == false (PR not already merged)
```

**If `mergeable_state` is NOT `"clean"`, DO NOT MERGE. Period.**

Additional requirements:
- [ ] `mergeable_state` is `"clean"` (not "unstable", "blocked", or "behind")
- [ ] Zero unaddressed bot comments on the latest commit
...
- `mcp__github__pull_request_read` - Get PR details, diff, status, comments
  ...
  - `method: "get_review_comments"` → Returns code review comments
  - `method: "get_comments"` → Returns PR conversation comments
- `mcp__github__merge_pull_request` - Merge PR (only when `mergeable_state` is `"clean"`)
